...
for PROJECT_ID in $PROJECT_IDS; do
        ...
        if [[ $PROJECT_INFO != "" ]]; then
                ...
                # Checking the project level configuration
                OSLOGIN_ENABLED_PROJECT="";
                if echo $PROJECT_INFO | jq -e '.commonInstanceMetadata.items // empty' > /dev/null; then
        ...
        if [[ $CSV != "True" ]]; then
                ...
                echo "\"$PROJECT_ID\", \"$PROJECT_NAME\", \"$PROJECT_OWNER\", \"$PROJECT_APPLICATION\", \"\", \"\", \"$VIOLATION_FLAG\", \"$OSLOGIN_ENABLED_PROJECT_STATUS_MESSAGE\"";
        fi;
